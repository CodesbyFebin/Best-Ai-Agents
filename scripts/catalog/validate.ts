...
import { entityKey } from "../../src/catalog/normalization";
...
for (const entity of catalog) {
  ...
  if (entity.providerId && !providers.some(p => p.id === entity.providerId)) throw new Error(`Unknown provider: ${entity.providerId}`);
  for (const alias of entity.aliases ?? []) { const key = `${entity.providerId}:${alias.toLowerCase()}`; if (aliases.has(key)) throw new Error(`Alias collision: ${key}`); aliases.add(key); }
